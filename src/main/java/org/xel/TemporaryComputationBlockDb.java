...
final class TemporaryComputationBlockDb {
    ...
    static BlockImpl findBlock(long blockId) {
        ...
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT * FROM block_comp WHERE id = ?")) {
    ...
    static boolean hasBlock(long blockId, int height) {
        ...
        // Search the database
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT height FROM block_comp WHERE id = ?")) {
            pstmt.setLong(1, blockId);
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    static long findBlockIdAtHeight(int height) {
        ...
        // Search the database
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT id FROM block_comp WHERE height = ?")) {
            pstmt.setInt(1, height);
            try (ResultSet rs = pstmt.executeQuery()) {
                if (!rs.next()) {
                    throw new RuntimeException("Block (Computational) at height " + height + " not found in database!");
                }
    ...
    static BlockImpl findBlockAtHeight(int height) {
        ...
        // Search the database
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT * FROM block_comp WHERE height = ?")) {
            pstmt.setInt(1, height);
            try (ResultSet rs = pstmt.executeQuery()) {
                BlockImpl block;
                if (rs.next()) {
    ...
    static BlockImpl findLastBlock() {
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT * FROM block_comp ORDER BY timestamp DESC LIMIT 1")) {
            BlockImpl block = null;
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    block = loadBlock(con, rs);
    ...
    static BlockImpl findLastBlock(int timestamp) {
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT * FROM block_comp WHERE timestamp <= ? ORDER BY timestamp DESC LIMIT 1")) {
    ...
    static Set<Long> getBlockGenerators(int startHeight) {
        ...
        try (Connection con = Db.db.getConnection();
                PreparedStatement pstmt = con.prepareStatement(
                        "SELECT generator_id, COUNT(generator_id) AS count FROM block_comp WHERE height >= ? GROUP BY generator_id")) {
    ...
    static void saveBlock(Connection con, BlockImpl block) {
        try {
            try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO block_comp (id, version, timestamp, previous_block_id, "
                ...
                pstmt.executeUpdate();
                TemporaryComputationTransactionDb.saveTransactions(con, block.getTransactions());
            }
            if (block.getPreviousBlockId() != 0) {
                try (PreparedStatement pstmt = con.prepareStatement("UPDATE block_comp SET next_block_id = ? WHERE id = ?")) {
    ...
    static void deleteBlocksFromHeight(int height) {
        ...
        TemporaryComputationBlockDb.deleteBlocksFrom(blockId);
    }
    ...
    static BlockImpl deleteBlocksFrom(long blockId) {
        ...
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmtSelect = con.prepareStatement("SELECT db_id FROM block_comp WHERE timestamp >= "
                     + "IFNULL ((SELECT timestamp FROM block_comp WHERE id = ?), " + Integer.MAX_VALUE + ") ORDER BY timestamp DESC");
             PreparedStatement pstmtDelete = con.prepareStatement("DELETE FROM block_comp WHERE db_id = ?")) {
            try {
                ...
                try (PreparedStatement pstmt = con.prepareStatement("UPDATE block_comp SET next_block_id = NULL WHERE id = ?")) {
