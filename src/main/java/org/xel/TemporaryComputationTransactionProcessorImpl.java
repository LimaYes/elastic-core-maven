...
public final class TemporaryComputationTransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final TemporaryComputationTransactionProcessorImpl instance = new TemporaryComputationTransactionProcessorImpl();

    public static TemporaryComputationTransactionProcessorImpl getInstance() {
        return instance;
    }
    ...
    final DbKey.LongKeyFactory<UnconfirmedTransaction> unconfirmedTransactionDbKeyFactory = new DbKey.LongKeyFactory<UnconfirmedTransaction>("id") {
        ...
        public DbKey newKey(UnconfirmedTransaction unconfirmedTransaction) {
            return unconfirmedTransaction.getTransaction().getDbKeyComputation();
        }

    };

    private final ComputationalEntityDbTable<UnconfirmedTransaction> unconfirmedTransactionTable = new ComputationalEntityDbTable<UnconfirmedTransaction>("unconfirmed_transaction_comp", unconfirmedTransactionDbKeyFactory) {
        ...
        public void rollback(int height) {
            try (Connection con = Db.db.getConnection();
                 PreparedStatement pstmt = con.prepareStatement("SELECT * FROM unconfirmed_transaction_comp WHERE height > ?")) {
                pstmt.setInt(1, height);
                try (ResultSet rs = pstmt.executeQuery()) {
                    while (rs.next()) {
                        UnconfirmedTransaction unconfirmedTransaction = load(con, rs, null);
                        waitingTransactions.add(unconfirmedTransaction);
                        transactionCache.remove(unconfirmedTransaction.getDbKeyComputation());
                    }
                }
        ...
        try {
            try {
                ...
                for (TransactionImpl transaction : broadcastedTransactions) {
                    if (transaction.getExpiration(true) < curTime || TemporaryComputationTransactionDb.hasTransaction(transaction.getId())) {
    ...
    private TemporaryComputationTransactionProcessorImpl() {
    ...
    private List<Long> getAllUnconfirmedTransactionIds() {
        ...
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT id FROM unconfirmed_transaction_comp");
    ...
    public void broadcast(Transaction transaction) throws NxtException.ValidationException {
        ...
        try {
            ...
            if (getUnconfirmedTransaction(((TransactionImpl)transaction).getDbKeyComputation()) != null) {
                if (enableTransactionRebroadcasting) {
                    broadcastedTransactions.add((TransactionImpl) transaction);
                    Logger.logMessage("Transaction (computational) " + transaction.getStringId() + " already in unconfirmed pool, will re-broadcast");
                } else {
                    Logger.logMessage("Transaction (computational) " + transaction.getStringId() + " already in unconfirmed pool, will not broadcast again");
                }
                return;
            }
    ...
    void removeUnconfirmedTransaction(TransactionImpl transaction) {
        ...
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("DELETE FROM unconfirmed_transaction_comp WHERE id = ?")) {
        ...
        for (Object transactionData : transactionsData) {
            try {
                ...
                if (broadcastedTransactions.contains(transaction)) {
                    Logger.logDebugMessage("Received back transaction (computational) " + transaction.getStringId()
    ...
    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws NxtException.ValidationException {
        ...
        try {
            try {
                Db.db.beginTransaction();
                if (Nxt.getTemporaryComputationBlockchain().getHeight() < Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
                    throw new NxtException.NotCurrentlyValidException("Blockchain (computational) not ready to accept transactions");
                }
                ...
                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    throw new NxtException.NotCurrentlyValidException("Duplicate unconfirmed transaction (computational(");
                }
