...
final class TemporaryComputationTransactionDb {
    ...
    static TransactionImpl findTransaction(long transactionId, int height) {
        ...
        synchronized (TemporaryComputationBlockDb.blockCache) {
            TransactionImpl transaction = TemporaryComputationBlockDb.transactionCache.get(transactionId);
            if (transaction != null) {
        ...
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT * FROM transaction_comp WHERE id = ?")) {
            ...
            throw new RuntimeException("Transaction (computational) already in database, id = " + transactionId + ", does not pass validation!", e);
        }
    }
    ...
    static TransactionImpl findTransactionByFullHash(byte[] fullHash, int height) {
        ...
        try (Connection con = Db.db.getConnection();
            ...
            throw new RuntimeException("Transaction (computational) already in database, full_hash = " + Convert.toHexString(fullHash)
    ...
    static boolean hasTransaction(long transactionId, int height) {
        ...
        // Search the database
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT height FROM transaction_comp WHERE id = ?")) {
            pstmt.setLong(1, transactionId);
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    static boolean hasTransactionByFullHash(byte[] fullHash, int height) {
        ...
        // Search the database
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT full_hash, height FROM transaction_comp WHERE id = ?")) {
            pstmt.setLong(1, transactionId);
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    static byte[] getFullHash(long transactionId) {
        ...
        // Search the database
        try (Connection con = Db.db.getConnection();
             PreparedStatement pstmt = con.prepareStatement("SELECT full_hash FROM transaction_comp WHERE id = ?")) {
            pstmt.setLong(1, transactionId);
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    static List<TransactionImpl> findBlockTransactions(Connection con, long blockId) {
        try (PreparedStatement pstmt = con.prepareStatement("SELECT * FROM transaction_comp WHERE block_id = ? ORDER BY transaction_index")) {
            ...
            throw new RuntimeException("Transaction (computational) already in database for block_id = " + Long.toUnsignedString(blockId)
                    + " does not pass validation!", e);
        }
    }
    ...
    static List<PrunableTransaction> findPrunableTransactions(Connection con, int minTimestamp, int maxTimestamp) {
        ...
        try (PreparedStatement pstmt = con.prepareStatement("SELECT id, type, subtype, "
                ...
                + "FROM transaction_comp WHERE (timestamp BETWEEN ? AND ?) AND "
    ...
    static void saveTransactions(Connection con, List<TransactionImpl> transactions) {
        try {
            ...
            for (TransactionImpl transaction : transactions) {
                try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO transaction_comp (id, deadline, "
                ...
                if (transaction.referencedTransactionFullHash() != null) {
                    try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO referenced_transaction_comp "
